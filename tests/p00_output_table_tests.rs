use lyra_phase0::p00::{
    parse_output_table_surface, validate_output_table_surface, ErrorCode,
    REQUIRED_OUTPUT_ARTIFACTS, REQUIRED_OUTPUT_AUDIENCES, REQUIRED_OUTPUT_CONTRACTS,
    REQUIRED_OUTPUT_RECEIPTS, REQUIRED_UNRESOLVED_GAPS,
};

fn fixture(name: &str) -> String {
    std::fs::read_to_string(format!("fixtures/p00/output_table_inputs/{name}"))
        .expect("fixture must exist")
}

#[test]
fn accepts_valid_output_table_surface() {
    let input = fixture("valid_output_table.lyra");
    let parsed = parse_output_table_surface(&input).expect("valid output table parses");
    assert_eq!(parsed.phase, "P00");
    assert_eq!(parsed.task, "P00-X04");
    assert_eq!(parsed.audiences.len(), REQUIRED_OUTPUT_AUDIENCES.len());
    assert_eq!(parsed.artifacts.len(), REQUIRED_OUTPUT_ARTIFACTS.len());
    assert_eq!(parsed.receipts.len(), REQUIRED_OUTPUT_RECEIPTS.len());
    assert_eq!(parsed.contracts.len(), REQUIRED_OUTPUT_CONTRACTS.len());
    assert_eq!(parsed.gaps.len(), REQUIRED_UNRESOLVED_GAPS.len());
...
#[test]
fn rejects_required_output_table_gaps() {
    for (fixture_name, expected) in [
        ...
        let (verdict, _) = validate_output_table_surface(&input);
        assert!(
...
#[test]
fn rejects_duplicate_unknown_and_invalid_output_bindings() {
    for (fixture_name, expected) in [
        (
