use lyra_phase0::p00::{
    parse_retirement_supersession_surface, validate_retirement_supersession_surface, ErrorCode,
    REQUIRED_RETIREMENT_GATES, REQUIRED_RETIREMENT_RECEIPTS, REQUIRED_RETIREMENT_SURFACES,
    REQUIRED_SUPERSESSIONS,
};

fn fixture(name: &str) -> String {
    std::fs::read_to_string(format!("fixtures/p00/retirement_inputs/{name}"))
        .expect("fixture must exist")
}

#[test]
fn accepts_valid_retirement_supersession_surface() {
    let input = fixture("valid_retirement_supersession.lyra");
    let parsed =
        parse_retirement_supersession_surface(&input).expect("valid retirement law parses");
    assert_eq!(parsed.phase, "P00");
    assert_eq!(parsed.task, "P00-X05");
    assert_eq!(parsed.surfaces.len(), REQUIRED_RETIREMENT_SURFACES.len());
    assert_eq!(parsed.gates.len(), REQUIRED_RETIREMENT_GATES.len());
    assert_eq!(parsed.supersessions.len(), REQUIRED_SUPERSESSIONS.len());
    assert_eq!(parsed.receipts.len(), REQUIRED_RETIREMENT_RECEIPTS.len());
...
#[test]
fn rejects_required_retirement_gaps() {
    for (fixture_name, expected) in [
        ...
        let (verdict, _) = validate_retirement_supersession_surface(&input);
        assert!(
...
#[test]
fn rejects_duplicate_unknown_and_unbound_retirement_rows() {
    for (fixture_name, expected) in [
        (
