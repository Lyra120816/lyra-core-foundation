use lyra_phase0::p01::{
    parse_semantic_output_table_surface, validate_semantic_output_table_surface, ErrorCode,
    REQUIRED_SEMANTIC_OUTPUT_ARTIFACTS, REQUIRED_SEMANTIC_OUTPUT_AUDIENCES,
    REQUIRED_SEMANTIC_OUTPUT_CONTRACTS, REQUIRED_SEMANTIC_OUTPUT_RECEIPTS,
    REQUIRED_SEMANTIC_UNRESOLVED_GAPS,
};

fn fixture(name: &str) -> String {
    std::fs::read_to_string(format!("fixtures/p01/semantic_output_table_inputs/{name}"))
        .expect("fixture must exist")
}

#[test]
fn accepts_valid_semantic_output_table_surface() {
    let input = fixture("valid_semantic_output_table.lyra");
    let parsed =
        parse_semantic_output_table_surface(&input).expect("valid semantic output table parses");
    assert_eq!(parsed.phase, "P01");
    assert_eq!(parsed.task, "P01-X04");
    assert_eq!(
        parsed.audiences.len(),
        REQUIRED_SEMANTIC_OUTPUT_AUDIENCES.len()
    );
    assert_eq!(
        parsed.artifacts.len(),
        REQUIRED_SEMANTIC_OUTPUT_ARTIFACTS.len()
    );
    assert_eq!(
        parsed.receipts.len(),
        REQUIRED_SEMANTIC_OUTPUT_RECEIPTS.len()
    );
    assert_eq!(
        parsed.contracts.len(),
        REQUIRED_SEMANTIC_OUTPUT_CONTRACTS.len()
    );
    assert_eq!(parsed.gaps.len(), REQUIRED_SEMANTIC_UNRESOLVED_GAPS.len());
    let (verdict, receipt) = validate_semantic_output_table_surface(&input);
    assert!(
        verdict.accepted,
...
#[test]
fn rejects_required_semantic_output_table_gaps() {
    for (fixture_name, expected) in [
        ...
        let (verdict, _) = validate_semantic_output_table_surface(&input);
        assert!(
...
#[test]
fn rejects_duplicate_unknown_and_invalid_semantic_output_bindings() {
    for (fixture_name, expected) in [
        (
