use lyra_phase0::p01::{
    deterministic_semantic_replay_suite_report, validate_semantic_replay_surface, ErrorCode,
...
const INVALID_MISSING_RULE: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_missing_replay_rule.lyra");
const INVALID_MISSING_RECEIPT: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_missing_receipt.lyra");
const INVALID_DUPLICATE_RECEIPT: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_duplicate_receipt.lyra");
const INVALID_BAD_HASH: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_bad_receipt_hash.lyra");
const INVALID_MISSING_WITNESS: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_missing_witness.lyra");
const INVALID_DUPLICATE_ORDER: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_duplicate_witness_order.lyra");
const INVALID_MISSING_LINK: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_missing_chain_link.lyra");
const INVALID_UNKNOWN_RECEIPT: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_unknown_receipt_reference.lyra");
const INVALID_MISSING_PROOF: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_missing_replay_proof.lyra");
const INVALID_UNBOUND_PROOF: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_unbound_proof_reference.lyra");
const INVALID_ORPHAN_RECEIPT: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_orphan_receipt.lyra");
const INVALID_MUTABLE_REPLAY: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_mutable_replay.lyra");
const INVALID_PHASE_CLOSURE: &str =
    include_str!("../fixtures/p01/semantic_replay_inputs/invalid_phase_closure_claim.lyra");

fn assert_rejects_with(input: &str, code: ErrorCode) {
    let (verdict, _receipt) = validate_semantic_replay_surface(input);
...
#[test]
fn accepts_valid_semantic_replay_surface() {
    let (verdict, receipt) = validate_semantic_replay_surface(VALID);
    assert!(
        verdict.accepted,
        "valid semantic replay rejected: {:?}",
        verdict.errors
    );
...
#[test]
fn semantic_replay_deterministic_report_is_stable_and_counted() {
    let receipts = vec![
        (
            "semantic_replay_receipt".to_string(),
            "receipts/p01/pass_0046_semantic_replay.receipt".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
        ...
        (
            "semantic_atoms_receipt".to_string(),
            "receipts/p01/pass_0030_semantic_atoms.receipt".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
            "fnv1a128:0123456789abcdef0123456789abcdef".to_string(),
    ...
    let witnesses = vec![
        (
            "z_witness".to_string(),
            "002".to_string(),
            vec!["semantic_replay_receipt".to_string()],
        ...
        (
            "a_witness".to_string(),
            "001".to_string(),
            vec!["semantic_atoms_receipt".to_string()],
    ...
    let artifacts = vec![(
        ...
        "k0".to_string(),
        "k0/determinism/src/semantic_replay.rs".to_string(),
        "deterministic_report".to_string(),
        "artifact_emitted".to_string(),
    )];
    let left = deterministic_semantic_replay_suite_report(
        &receipts, &witnesses, &links, &proofs, &artifacts,
    );
    let right = deterministic_semantic_replay_suite_report(
        &receipts, &witnesses, &links, &proofs, &artifacts,
    );
...
#[test]
fn required_semantic_replay_inventory_is_broad() {
    assert_eq!(REQUIRED_SEMANTIC_REPLAY_RECEIPTS.len(), 17);
    assert_eq!(REQUIRED_SEMANTIC_REPLAY_WITNESSES.len(), 7);
    assert_eq!(REQUIRED_SEMANTIC_REPLAY_LINKS.len(), 16);
    assert_eq!(REQUIRED_SEMANTIC_REPLAY_PROOFS.len(), 5);
    assert_eq!(REQUIRED_SEMANTIC_REPLAY_ARTIFACTS.len(), 7);
}
