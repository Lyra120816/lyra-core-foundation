...
pub struct BootstrapEconomicsFrameReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapPublicInterestOutputReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapEconomicsProofReport {
    pub id: String,
    pub scope: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapEconomicsSuiteReport {
    pub frame_count: usize,
    pub output_count: usize,
    pub proof_count: usize,
    pub frame_reports: Vec<BootstrapEconomicsFrameReport>,
    pub output_reports: Vec<BootstrapPublicInterestOutputReport>,
    pub proof_reports: Vec<BootstrapEconomicsProofReport>,
    pub suite_hash: String,
}

pub fn deterministic_bootstrap_economics_suite_report(
    frames: &[(
        String,
        String,
        String,
    ...
    let mut frame_reports: Vec<_> = frames
        ...
        .map(|item| {
            ...
            BootstrapEconomicsFrameReport {
                ...
                frame_hash: stable_hash_label("lyra.p02.bootstrap_economics.frame", &preimage),
            }
        })
        .collect();
    ...
    let mut output_reports: Vec<_> = outputs.iter().map(|item| {
        ...
        BootstrapPublicInterestOutputReport {
            id: item.0.clone(), output_kind: item.1.clone(), path: item.2.clone(),
            constituency_count: sorted_count(&item.3), command_count: sorted_count(&item.4), proof_count: sorted_count(&item.5),
            receipt_count: sorted_count(&item.6), rejection_count: sorted_count(&item.7),
            output_hash: stable_hash_label("lyra.p02.bootstrap_economics.output", &preimage),
        }
    }).collect();
    ...
    let mut proof_reports: Vec<_> = proofs.iter().map(|item| {
        ...
        BootstrapEconomicsProofReport {
            id: item.0.clone(), scope: item.1.clone(), frame_count: sorted_count(&item.2), output_count: sorted_count(&item.3),
            receipt_count: sorted_count(&item.4), command_count: sorted_count(&item.5), forbid_count: sorted_count(&item.6),
            proof_hash: stable_hash_label("lyra.p02.bootstrap_economics.proof", &preimage),
        }
    }).collect();
    ...
    BootstrapEconomicsSuiteReport {
        ...
        output_reports,
        proof_reports,
        suite_hash: stable_hash_label("lyra.p02.bootstrap_economics.suite", &suite_rows.join("\n")),
    }
}
