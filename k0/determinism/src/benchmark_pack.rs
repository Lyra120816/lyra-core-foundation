...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BenchmarkPackReport {
    ...
    pub family_reports: Vec<BenchmarkFamilyReport>,
    pub target_reports: Vec<BenchmarkTargetReport>,
    pub evidence_reports: Vec<BenchmarkEvidenceReport>,
    pub pack_hash: String,
}

pub fn deterministic_benchmark_pack_report(
    ...
    for (id, family_kind, mut target_ids, mut proofs, status) in sorted_families {
        ...
        let family_hash = stable_hash_label("lyra.p00.benchmark_pack.family", &family_preimage);
        preimage.push('|');
    ...
    for (id, family, metric, unit, threshold, command, receipt, status) in sorted_targets {
        match family.as_str() {
        ...
        let target_preimage = format!(
            "target:{id}|family:{family}|metric:{metric}|unit:{unit}|threshold:{threshold}|command:{command}|receipt:{receipt}|status:{status}"
        );
        let target_hash = stable_hash_label("lyra.p00.benchmark_pack.target", &target_preimage);
        preimage.push('|');
        preimage.push_str(&target_preimage);
        target_reports.push(BenchmarkTargetReport {
            id,
            family,
            metric,
            unit,
    ...
    for (id, family, mut target_ids, mut artifacts, mut proof_receipts, status) in sorted_evidence {
        ...
        let evidence_hash =
            stable_hash_label("lyra.p00.benchmark_pack.evidence", &evidence_preimage);
        preimage.push('|');
        preimage.push_str(&evidence_preimage);
        evidence_reports.push(BenchmarkEvidenceReport {
            id,
            family,
            target_count: target_ids.len(),
    ...
    BenchmarkPackReport {
        ...
        target_reports,
        evidence_reports,
        pack_hash: stable_hash_label("lyra.p00.benchmark_pack.pack", &preimage),
    }
}
