...
pub struct SemanticOutputAudienceReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticOutputArtifactReport {
    pub id: String,
    pub audience: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticOutputReceiptReport {
    pub id: String,
    pub path: String,
    pub target: String,
    pub status: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticOutputContractReport {
    pub id: String,
    pub surface: String,
    pub path: String,
    pub status: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticOutputGapReport {
    pub id: String,
    pub blocker: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticOutputTableReport {
    ...
    pub audience_reports: Vec<SemanticOutputAudienceReport>,
    pub artifact_reports: Vec<SemanticOutputArtifactReport>,
    pub receipt_reports: Vec<SemanticOutputReceiptReport>,
    pub contract_reports: Vec<SemanticOutputContractReport>,
    pub gap_reports: Vec<SemanticOutputGapReport>,
    pub table_hash: String,
}

pub fn deterministic_semantic_output_table_report(
    ...
    for (id, mut outputs, mut artifacts, mut receipts, status) in sorted_audiences {
        ...
        let audience_preimage = format!(
            "semantic-audience:{}|outputs:{}|artifacts:{}|receipts:{}|status:{}",
            id,
            outputs.join(","),
            artifacts.join(","),
            receipts.join(","),
        ...
        let audience_hash = stable_hash_label(
            "lyra.p01.semantic.output_table.audience",
            &audience_preimage,
        );
        preimage.push('|');
        preimage.push_str(&audience_preimage);
        audience_reports.push(SemanticOutputAudienceReport {
            id,
    ...
    for (id, audience, artifact_kind, path, status) in sorted_artifacts {
        ...
        let artifact_preimage = format!("semantic-artifact:{id}|audience:{audience}|kind:{artifact_kind}|path:{path}|status:{status}");
        let artifact_hash = stable_hash_label(
            "lyra.p01.semantic.output_table.artifact",
            &artifact_preimage,
        );
        preimage.push('|');
        preimage.push_str(&artifact_preimage);
        artifact_reports.push(SemanticOutputArtifactReport {
            id,
            audience,
            artifact_kind,
            path,
    ...
    for (id, path, target, status) in sorted_receipts {
        let receipt_preimage =
            format!("semantic-receipt:{id}|path:{path}|target:{target}|status:{status}");
        let receipt_hash =
            stable_hash_label("lyra.p01.semantic.output_table.receipt", &receipt_preimage);
        preimage.push('|');
        preimage.push_str(&receipt_preimage);
        receipt_reports.push(SemanticOutputReceiptReport {
            id,
            path,
            target,
            status,
    ...
    for (id, surface, path, status) in sorted_contracts {
        let contract_preimage =
            format!("semantic-contract:{id}|surface:{surface}|path:{path}|status:{status}");
        let contract_hash = stable_hash_label(
            "lyra.p01.semantic.output_table.contract",
            &contract_preimage,
        );
        preimage.push('|');
        preimage.push_str(&contract_preimage);
        contract_reports.push(SemanticOutputContractReport {
            id,
            surface,
            path,
            status,
    ...
    for (id, blocker, next_frontier, owner_root, status) in sorted_gaps {
        let gap_preimage = format!("semantic-gap:{id}|blocker:{blocker}|next:{next_frontier}|owner:{owner_root}|status:{status}");
        let gap_hash = stable_hash_label("lyra.p01.semantic.output_table.gap", &gap_preimage);
        preimage.push('|');
        preimage.push_str(&gap_preimage);
        gap_reports.push(SemanticOutputGapReport {
            id,
            blocker,
            next_frontier,
            owner_root,
    ...
    SemanticOutputTableReport {
        ...
        contract_reports,
        gap_reports,
        table_hash: stable_hash_label("lyra.p01.semantic.output_table.table", &preimage),
    }
}
