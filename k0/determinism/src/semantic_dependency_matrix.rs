...
pub struct SemanticDependencyNodeReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticBlockerBindingReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticParallelLaneReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticDependencyMatrixReport {
    pub dependency_count: usize,
    pub blocker_count: usize,
    pub lane_count: usize,
    pub open_blocker_count: usize,
    pub emitted_output_count: usize,
    pub node_reports: Vec<SemanticDependencyNodeReport>,
    pub blocker_reports: Vec<SemanticBlockerBindingReport>,
    pub lane_reports: Vec<SemanticParallelLaneReport>,
    pub matrix_hash: String,
}

pub fn deterministic_semantic_dependency_matrix_report(
    dependencies: &[(
        String,
        String,
        Vec<String>,
...
) -> SemanticDependencyMatrixReport {
        ...
        sorted_dependencies
    {
        ...
        unlocks.sort();
        receipts.sort();
        if id.starts_with("P01-X") && status == "artifact_emitted" {
        ...
        let node_hash = stable_hash_label("lyra.p01.semantic.dependency.node", &node_preimage);
        preimage.push('|');
        preimage.push_str(&node_preimage);
        node_reports.push(SemanticDependencyNodeReport {
            id,
            node_kind,
    ...
    for (id, mut blocked_by, reason, mut unblocks, mut receipts, status) in sorted_blockers {
        ...
        let blocker_hash =
            stable_hash_label("lyra.p01.semantic.dependency.blocker", &blocker_preimage);
        preimage.push('|');
        preimage.push_str(&blocker_preimage);
        blocker_reports.push(SemanticBlockerBindingReport {
            id,
        ...
        sorted_lanes
    {
        ...
        let lane_hash = stable_hash_label("lyra.p01.semantic.dependency.lane", &lane_preimage);
        preimage.push('|');
        preimage.push_str(&lane_preimage);
        lane_reports.push(SemanticParallelLaneReport {
            id,
            lane_kind,
    ...
    SemanticDependencyMatrixReport {
        ...
        blocker_reports,
        lane_reports,
        matrix_hash: stable_hash_label("lyra.p01.semantic.dependency.matrix", &preimage),
    }
}
