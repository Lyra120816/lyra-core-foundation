...
pub struct BootstrapProofFamilyReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapProofReceiptReport {
    pub id: String,
    pub family: String,
    pub path: String,
    pub coverage_count: usize,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapProofPathReport {
    pub id: String,
    pub family: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapProofFamilyTableReport {
    ...
    pub dependency_path_receipt_count: usize,
    pub family_reports: Vec<BootstrapProofFamilyReport>,
    pub receipt_reports: Vec<BootstrapProofReceiptReport>,
    pub path_reports: Vec<BootstrapProofPathReport>,
    pub table_hash: String,
}

pub fn deterministic_bootstrap_proof_family_table_report(
    families: &[(
        String,
        String,
        Vec<String>,
...
) -> BootstrapProofFamilyTableReport {
    ...
    for (id, family_kind, mut family_receipts, mut covers, mut proofs, status) in sorted_families {
        ...
        let family_hash =
            stable_hash_label("lyra.p02.bootstrap_proof_family.family", &family_preimage);
        rows.push(format!("family:{id}|hash:{family_hash}"));
        family_reports.push(BootstrapProofFamilyReport {
            id,
            family_kind,
    ...
    for (id, family, path, mut covers, verdict, status) in sorted_receipts {
        ...
        match family.as_str() {
            ...
            "dependency_path" => dependency_path_receipt_count += 1,
            _ => {}
        }
        ...
        let receipt_hash =
            stable_hash_label("lyra.p02.bootstrap_proof_family.receipt", &receipt_preimage);
        rows.push(format!("receipt:{id}|hash:{receipt_hash}"));
        receipt_reports.push(BootstrapProofReceiptReport {
            id,
            family,
            path,
            coverage_count: covers.len(),
    ...
    {
        entry_receipts.sort();
        entry_receipts.dedup();
        challenge_receipts.sort();
        challenge_receipts.dedup();
        rollback_receipts.sort();
        rollback_receipts.dedup();
        ...
        let path_hash = stable_hash_label("lyra.p02.bootstrap_proof_family.path", &path_preimage);
        rows.push(format!("path:{id}|hash:{path_hash}"));
        path_reports.push(BootstrapProofPathReport {
            id,
            family,
            path_kind,
    ...
    BootstrapProofFamilyTableReport {
        ...
        dependency_path_receipt_count,
        family_reports,
        receipt_reports,
        path_reports,
        table_hash: stable_hash_label("lyra.p02.bootstrap_proof_family.table", &rows.join("\n")),
    }
}
