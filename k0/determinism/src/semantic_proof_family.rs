...
pub struct SemanticProofFamilyReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticProofReceiptReport {
    pub id: String,
    pub family: String,
    pub path: String,
    pub coverage_count: usize,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticProofPathReport {
    pub id: String,
    pub family: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SemanticProofFamilyTableReport {
    ...
    pub family_reports: Vec<SemanticProofFamilyReport>,
    pub receipt_reports: Vec<SemanticProofReceiptReport>,
    pub path_reports: Vec<SemanticProofPathReport>,
    pub table_hash: String,
}

pub fn deterministic_semantic_proof_family_table_report(
    families: &[(
        String,
        String,
        Vec<String>,
...
) -> SemanticProofFamilyTableReport {
    ...
    for (id, family_kind, mut family_receipts, mut covers, mut proofs, status) in sorted_families {
        ...
        let family_hash =
            stable_hash_label("lyra.p01.semantic.proof_family.family", &family_preimage);
        preimage.push('|');
        preimage.push_str(&family_preimage);
        family_reports.push(SemanticProofFamilyReport {
            id,
            family_kind,
    ...
    for (id, family, path, mut covers, verdict, status) in sorted_receipts {
        ...
        let receipt_hash =
            stable_hash_label("lyra.p01.semantic.proof_family.receipt", &receipt_preimage);
        preimage.push('|');
        preimage.push_str(&receipt_preimage);
        receipt_reports.push(SemanticProofReceiptReport {
            id,
            family,
            path,
            coverage_count: covers.len(),
    ...
    ) in sorted_paths
    {
        ...
        let path_hash = stable_hash_label("lyra.p01.semantic.proof_family.path", &path_preimage);
        preimage.push('|');
        preimage.push_str(&path_preimage);
        path_reports.push(SemanticProofPathReport {
            id,
            family,
            path_kind,
    ...
    SemanticProofFamilyTableReport {
        ...
        receipt_reports,
        path_reports,
        table_hash: stable_hash_label("lyra.p01.semantic.proof_family.table", &preimage),
    }
}
