...
pub struct BootstrapDeploymentTargetReport {
    pub id: String,
    pub kind: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapComplianceHookReport {
    pub id: String,
    pub scope: String,
    pub target: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapReleaseEvidenceReport {
    pub id: String,
    pub kind: String,
    pub path: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapDeploymentProofReport {
    pub id: String,
    pub scope: String,
    pub target_count: usize,
    pub hook_count: usize,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BootstrapDeploymentSuiteReport {
    pub target_count: usize,
    pub hook_count: usize,
    pub evidence_count: usize,
    pub proof_count: usize,
    pub targets: Vec<BootstrapDeploymentTargetReport>,
    pub hooks: Vec<BootstrapComplianceHookReport>,
    pub evidence: Vec<BootstrapReleaseEvidenceReport>,
    pub proofs: Vec<BootstrapDeploymentProofReport>,
    pub suite_hash: String,
}

pub fn deterministic_bootstrap_deployment_suite_report(
    targets: &[(
        String,
        String,
        String,
...
) -> BootstrapDeploymentSuiteReport {
    let mut target_reports: Vec<_> = targets.iter().map(|item| {
        ...
        BootstrapDeploymentTargetReport { id: item.0.clone(), kind: item.1.clone(), environment: item.2.clone(), artifact_count: sorted_count(&item.3), command_count: sorted_count(&item.4), receipt_count: sorted_count(&item.5), forbid_count: sorted_count(&item.6), target_hash: stable_hash_label("lyra.p02.bootstrap_deployment.target", &preimage) }
    }).collect();
    ...
    let mut hook_reports: Vec<_> = hooks
        ...
        .map(|item| {
            ...
            BootstrapComplianceHookReport {
                ...
                hook_hash: stable_hash_label("lyra.p02.bootstrap_deployment.hook", &preimage),
            }
        })
        .collect();
    ...
    let mut evidence_reports: Vec<_> = evidence
        ...
        .map(|item| {
            ...
            BootstrapReleaseEvidenceReport {
                id: item.0.clone(),
                kind: item.1.clone(),
                path: item.2.clone(),
    ...
    let mut proof_reports: Vec<_> = proofs.iter().map(|item| {
        ...
        BootstrapDeploymentProofReport { id: item.0.clone(), scope: item.1.clone(), target_count: sorted_count(&item.2), hook_count: sorted_count(&item.3), evidence_count: sorted_count(&item.4), receipt_count: sorted_count(&item.5), command_count: sorted_count(&item.6), forbid_count: sorted_count(&item.7), proof_hash: stable_hash_label("lyra.p02.bootstrap_deployment.proof", &preimage) }
    }).collect();
    ...
    BootstrapDeploymentSuiteReport {
        ...
        suite_hash: stable_hash_label(
            "lyra.p02.bootstrap_deployment.suite",
            &suite_rows.join("\n"),
        ),
    }
}
