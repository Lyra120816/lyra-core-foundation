...
pub struct RetirementSurfaceReport {
    pub id: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RetirementGateReport {
    pub id: String,
    pub surface: String,
    pub trigger: String,
    pub action: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RetirementReceiptReport {
    pub id: String,
    pub path: String,
    pub target: String,
    pub status: String,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RetirementSupersessionReport {
    ...
    pub surface_reports: Vec<RetirementSurfaceReport>,
    pub gate_reports: Vec<RetirementGateReport>,
    pub supersession_reports: Vec<SupersessionReport>,
    pub receipt_reports: Vec<RetirementReceiptReport>,
    pub law_hash: String,
}

pub fn deterministic_retirement_supersession_report(
    ...
    for (id, owner_root, surface_kind, path, status) in sorted_surfaces {
        ...
        let surface_preimage = format!(
            "surface:{id}|owner:{owner_root}|kind:{surface_kind}|path:{path}|status:{status}"
        );
        let surface_hash = stable_hash_label("lyra.p00.retirement.surface", &surface_preimage);
        preimage.push('|');
        preimage.push_str(&surface_preimage);
        surface_reports.push(RetirementSurfaceReport {
            id,
            owner_root,
            surface_kind,
            path,
    ...
    for (id, surface, trigger, action, mut evidence, status) in sorted_gates {
        ...
        let gate_preimage = format!(
            "gate:{}|surface:{}|trigger:{}|action:{}|evidence:{}|status:{}",
            id,
            surface,
            trigger,
            action,
        ...
        let gate_hash = stable_hash_label("lyra.p00.retirement.gate", &gate_preimage);
        preimage.push('|');
        preimage.push_str(&gate_preimage);
        gate_reports.push(RetirementGateReport {
            id,
            surface,
            trigger,
            action,
    ...
    for (id, surface, replaced_by, archive, status) in sorted_supersessions {
        let supersession_preimage = format!("supersession:{id}|surface:{surface}|replaced_by:{replaced_by}|archive:{archive}|status:{status}");
        let supersession_hash =
            stable_hash_label("lyra.p00.retirement.supersession", &supersession_preimage);
        preimage.push('|');
        preimage.push_str(&supersession_preimage);
        supersession_reports.push(SupersessionReport {
            id,
            surface,
            replaced_by,
            archive,
    ...
    for (id, path, target, status) in sorted_receipts {
        ...
        let receipt_hash = stable_hash_label("lyra.p00.retirement.receipt", &receipt_preimage);
        preimage.push('|');
        preimage.push_str(&receipt_preimage);
        receipt_reports.push(RetirementReceiptReport {
            id,
            path,
            target,
            status,
    ...
    RetirementSupersessionReport {
        ...
        receipt_reports,
        law_hash: stable_hash_label("lyra.p00.retirement.law", &preimage),
    }
}
