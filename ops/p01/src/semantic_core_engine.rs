...
use crate::k0_semantic_core_engine::deterministic_semantic_core_engine_suite_report;
use crate::k0_verdict::{ErrorCode, ValidationError, Verdict};
use crate::lyralang_canonical_data_model::canonical_data_model_descriptor;
use crate::lyralang_semantic_core_engine::{
    semantic_core_engine_artifact_descriptor, semantic_core_engine_artifact_digest,
    semantic_core_engine_artifact_ids, semantic_core_engine_artifacts_bind_paths,
    ...
    semantic_core_engine_proofs_bind_registry, semantic_core_engine_registry_hash,
    ...
    LYRA_P01_SEMANTIC_CORE_ENGINE_CARRIER,
};
use crate::p01_semantic_core_engine_model::{
    SemanticCoreEngineArtifactBinding, SemanticCoreEngineProofBinding, SemanticCoreEngineSurface,
    SemanticCoreEngineTransitionBinding, SemanticCoreEngineUnitBinding,
};

pub const P01_SEMANTIC_CORE_ENGINE_CONTRACT: &str = "LYRA-P01-SEMANTIC-CORE-ENGINE v1";
pub const REQUIRED_SEMANTIC_CORE_ENGINE_RULES: &[&str] = &[
    "canonical_symbol_engine_total",
    "semantic_atom_engine_closed",
    "core_ir_engine_single_carrier",
    "engine_units_ordered",
    "transitions_total_and_bound",
    "outputs_bind_canonical_models",
    "artifacts_bind_owner_roots",
    "proofs_bind_fixture_golden_receipt",
    "receipt_commit_is_deterministic",
    "replay_witness_is_receipt_bound",
    "no_network_engine_source",
    "no_probabilistic_engine",
    "no_hidden_randomness",
    "no_phase_closure_claim",
];
pub const REQUIRED_SEMANTIC_CORE_ENGINE_UNITS: &[&str] = &[
...
pub const REQUIRED_SEMANTIC_CORE_ENGINE_TRANSITIONS: &[&str] = &[
...
pub const REQUIRED_SEMANTIC_CORE_ENGINE_ARTIFACTS: &[&str] = &[
    "engine_contract",
    "engine_law",
    "engine_operator",
...
pub const REQUIRED_SEMANTIC_CORE_ENGINE_PROOFS: &[&str] = &[
    "unit_order_proof",
    "model_binding_proof",
    "transition_totality_proof",
    "artifact_binding_proof",
    "receipt_commit_proof",
    "p01_semantic_core_engine_parity_proof",
];
...
pub fn parse_semantic_core_engine_surface(
    input: &str,
) -> Result<SemanticCoreEngineSurface, Vec<ValidationError>> {
    ...
    if lines.is_empty() {
        return Err(vec![ValidationError::reject(
            ...
            "input",
            "empty semantic core engine surface",
        )]);
    }

    let header = lines[0].clone();
    let mut errors = Vec::new();
    if header != P01_SEMANTIC_CORE_ENGINE_CONTRACT {
        errors.push(ValidationError::reject(
            ErrorCode::InvalidHeader,
            "line:001",
            format!("expected {P01_SEMANTIC_CORE_ENGINE_CONTRACT}"),
        ));
    }
    ...
    for (index, line) in lines.iter().enumerate().skip(1) {
            ...
            .and_then(|value| value.split_once('='))
        {
            ...
            units.push(SemanticCoreEngineUnitBinding {
                line_number,
            ...
            transitions.push(SemanticCoreEngineTransitionBinding {
                line_number,
                id: field(&field_map, "id"),
            ...
            artifacts.push(SemanticCoreEngineArtifactBinding {
                line_number,
                id: field(&field_map, "id"),
                owner_root: field(&field_map, "owner"),
            ...
            proofs.push(SemanticCoreEngineProofBinding {
                line_number,
                id: field(&field_map, "id"),
            ...
            errors.push(ValidationError::reject(
                ...
                format!("line:{line_number:03}"),
                format!("unrecognized semantic core engine line {line}"),
            ));
        }
    }
...
pub fn validate_semantic_core_engine_surface(input: &str) -> (Verdict, Receipt) {
    let canonical = canonical_surface_text(input).unwrap_or_else(|_| String::new());
    let mut errors = Vec::new();
    scan_forbidden_text(&canonical, &mut errors);
    let parsed = match parse_semantic_core_engine_surface(input) {
        Ok(surface) => surface,
    ...
    validate_semantic_core_engine(&parsed, &mut errors);
...
fn validate_semantic_core_engine(
    surface: &SemanticCoreEngineSurface,
    ...
    if surface.task != "P01-015" {
        errors.push(ValidationError::reject(
            ErrorCode::InvalidTask,
            "task",
            format!("expected P01-015 got {}", surface.task),
        ));
    }
    ...
    for required in REQUIRED_SEMANTIC_CORE_ENGINE_RULES {
        match surface.rules.get(*required) {
            ...
            None => errors.push(ValidationError::reject(
                ErrorCode::MissingEngineRule,
                format!("rule:{required}"),
                "missing semantic core engine rule",
            )),
        }
    }
    ...
    require_ids(
        "unit",
        REQUIRED_SEMANTIC_CORE_ENGINE_UNITS,
    ...
    require_ids(
        "transition",
        REQUIRED_SEMANTIC_CORE_ENGINE_TRANSITIONS,
        surface
            .transitions
            .iter()
    ...
    require_ids(
        "artifact",
        REQUIRED_SEMANTIC_CORE_ENGINE_ARTIFACTS,
        surface
            .artifacts
            .iter()
            .map(|item| item.id.as_str())
    ...
    require_ids(
        "proof",
        REQUIRED_SEMANTIC_CORE_ENGINE_PROOFS,
    ...
    for unit in &surface.units {
        ...
        let Some(descriptor) = semantic_core_engine_unit_descriptor(&unit.id) else {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidEngineUnit,
                format!("line:{:03}", unit.line_number),
                format!("unknown semantic core engine unit {}", unit.id),
            ));
            continue;
        };
        ...
        if semantic_core_engine_unit_digest(&unit.id).is_none() {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidEngineUnit,
                format!("line:{:03}", unit.line_number),
    ...
    for transition in &surface.transitions {
        ...
        let Some(descriptor) = semantic_core_engine_transition_descriptor(&transition.id) else {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidTransitionBinding,
                format!("line:{:03}", transition.line_number),
                format!("unknown semantic core engine transition {}", transition.id),
            ));
            continue;
        };
        ...
        if semantic_core_engine_transition_digest(&transition.id).is_none() {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidTransitionBinding,
                format!("line:{:03}", transition.line_number),
    ...
    for artifact in &surface.artifacts {
        ...
        let Some(descriptor) = semantic_core_engine_artifact_descriptor(&artifact.id) else {
            errors.push(ValidationError::reject(
                ErrorCode::UnknownEvidencePath,
                format!("line:{:03}", artifact.line_number),
                format!("unknown semantic core engine artifact {}", artifact.id),
            ));
            continue;
        };
        ...
        if semantic_core_engine_artifact_digest(&artifact.id).is_none() {
            errors.push(ValidationError::reject(
                ErrorCode::UnknownEvidencePath,
                format!("line:{:03}", artifact.line_number),
    ...
    for proof in &surface.proofs {
        ...
        let Some(descriptor) = semantic_core_engine_proof_descriptor(&proof.id) else {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidEngineProof,
                format!("line:{:03}", proof.line_number),
                format!("unknown semantic core engine proof {}", proof.id),
            ));
            continue;
        };
        if proof.units
        ...
        if semantic_core_engine_proof_digest(&proof.id).is_none() {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidEngineProof,
                format!("line:{:03}", proof.line_number),
    ...
    if !semantic_core_engine_units_have_stable_order()
        || !semantic_core_engine_transitions_bind_known_units()
        || !semantic_core_engine_artifacts_bind_paths()
        || !semantic_core_engine_proofs_bind_registry()
        || !semantic_core_engine_no_forbidden_descriptor_claims()
    {
        errors.push(ValidationError::reject(
            ErrorCode::EngineDriftAccepted,
            "semantic_core_engine",
            "semantic core engine descriptor registry is incomplete or drifted",
        ));
    }
    ...
    let proof_rows: Vec<(
        ...
        .collect();
    let suite = deterministic_semantic_core_engine_suite_report(
        &unit_rows,
        &transition_rows,
        &artifact_rows,
        &proof_rows,
    );
    if suite.unit_count < semantic_core_engine_unit_ids().len()
        || suite.transition_count < semantic_core_engine_transition_ids().len()
        || suite.artifact_count < semantic_core_engine_artifact_ids().len()
        || suite.proof_count < semantic_core_engine_proof_ids().len()
        || suite.lyralang_owned_count == 0
        || suite.k0_owned_count == 0
        || !suite.suite_hash.starts_with("fnv1a128:")
        || !semantic_core_engine_registry_hash().starts_with("fnv1a128:")
        || LYRA_P01_SEMANTIC_CORE_ENGINE_CARRIER != "lyra_p01_semantic_core_engine"
    {
        errors.push(ValidationError::reject(
            ErrorCode::InvalidEngineSurface,
            "suite",
            "semantic core engine suite report is incomplete or unhashable",
        ));
    }
}
...
fn validate_model_endpoint(
    ...
    if model == "operator_surface_bytes" || model == "p01_replay_witness" {
        return;
    }
    if canonical_data_model_descriptor(model).is_none() {
        errors.push(ValidationError::reject(
            ErrorCode::EngineProofUnbound,
            format!("line:{line_number:03}"),
            format!("{kind} model {model} is not a canonical data model endpoint"),
        ));
    }
}
    ...
    for id in required {
        if !actual.contains(id) {
            errors.push(ValidationError::reject(
                code,
                format!("{kind}:{id}"),
                format!("missing required semantic core engine {kind} {id}"),
            ));
        }
    }
}
    ...
    for (id, line_number) in items {
        if !seen.insert(id.to_string()) {
            errors.push(ValidationError::reject(
                ErrorCode::DuplicateEntry,
                format!("line:{line_number:03}"),
                format!("duplicate semantic core engine {kind} {id}"),
            ));
        }
    }
}
