...
use crate::k0_bootstrap_deployment::deterministic_bootstrap_deployment_suite_report;
use crate::k0_canonical::{canonical_lines, canonical_surface_text};
use crate::k0_receipt::{build_phase_receipt, Receipt};
use crate::k0_verdict::{ErrorCode, ValidationError, Verdict};
use crate::lyralang_bootstrap_deployment::{
    ...
    bootstrap_deployment_evidence_digest, bootstrap_deployment_hook_descriptor,
    bootstrap_deployment_hook_digest, bootstrap_deployment_hooks_bind_targets,
    bootstrap_deployment_no_forbidden_descriptor_claims, bootstrap_deployment_proof_descriptor,
    bootstrap_deployment_proof_digest, bootstrap_deployment_proofs_bind_registry,
    bootstrap_deployment_receipts_cover_p02_001_through_p02_020,
    bootstrap_deployment_registry_hash, bootstrap_deployment_target_descriptor,
    bootstrap_deployment_target_digest,
};
use crate::p02_bootstrap_deployment_model::{
...
pub const P02_BOOTSTRAP_DEPLOYMENT_CONTRACT: &str = "LYRA-P02-BOOTSTRAP-DEPLOYMENT-HOOKS v1";

pub const REQUIRED_BOOTSTRAP_DEPLOYMENT_RULES: &[&str] = &[
    "bootstrap_deployment_manifest_required",
    ...
    "bootstrap_packaging_bridge_required",
    "host_extinction_gate_required",
    "seed_runtime_gate_required",
...
pub const REQUIRED_BOOTSTRAP_DEPLOYMENT_TARGETS: &[&str] = &[
...
pub const REQUIRED_BOOTSTRAP_COMPLIANCE_HOOKS: &[&str] = &[
    "bootstrap_artifact_inventory_gate",
    "bootstrap_receipt_chain_gate",
...
pub const REQUIRED_BOOTSTRAP_RELEASE_EVIDENCE: &[&str] = &[
...
pub const REQUIRED_BOOTSTRAP_DEPLOYMENT_PROOFS: &[&str] = &[
    "bootstrap_target_coverage",
    "bootstrap_offline_deployment_gate",
    "bootstrap_enterprise_compliance_binding",
    "bootstrap_release_evidence_replay",
    "bootstrap_packaging_bridge",
    "p02_phase_open",
];
...
pub fn parse_bootstrap_deployment_surface(
    input: &str,
) -> Result<BootstrapDeploymentSurface, Vec<ValidationError>> {
    ...
    if lines.is_empty() {
        return Err(vec![ValidationError::reject(
            ...
            "line:000",
            "no bootstrap deployment surface lines",
        )]);
    }
    let header = lines[0].clone();
    if header != P02_BOOTSTRAP_DEPLOYMENT_CONTRACT {
        return Err(vec![ValidationError::reject(
            ErrorCode::InvalidHeader,
            "line:001",
            format!("expected {P02_BOOTSTRAP_DEPLOYMENT_CONTRACT}"),
        )]);
    }
    ...
    for (offset, line) in lines.iter().enumerate().skip(1) {
        ...
        if let Some(rule_name) = left.strip_prefix("rule:") {
            if !is_symbolic_name(rule_name) || !seen_rules.insert(rule_name.to_string()) {
                errors.push(ValidationError::reject(
                    ErrorCode::InvalidEntrySyntax,
                    format!("line:{line_number:03}"),
                    "bootstrap deployment rule names must be symbolic and unique",
                ));
            } else {
        ...
        if let Some(target_id) = left.strip_prefix("target:") {
            if !is_symbolic_name(target_id) || !seen_targets.insert(target_id.to_string()) {
                errors.push(ValidationError::reject(
                    ...
                    format!("line:{line_number:03}"),
                    format!("duplicate or invalid bootstrap target {target_id}"),
                ));
                continue;
            }
        ...
        if let Some(hook_id) = left.strip_prefix("hook:") {
            if !is_symbolic_name(hook_id) || !seen_hooks.insert(hook_id.to_string()) {
                errors.push(ValidationError::reject(
                    ...
                    format!("line:{line_number:03}"),
                    format!("duplicate or invalid bootstrap hook {hook_id}"),
                ));
                continue;
            }
        ...
        if let Some(evidence_id) = left.strip_prefix("evidence:") {
            if !is_symbolic_name(evidence_id) || !seen_evidence.insert(evidence_id.to_string()) {
                errors.push(ValidationError::reject(
                    ...
                    format!("line:{line_number:03}"),
                    format!("duplicate or invalid bootstrap evidence {evidence_id}"),
                ));
                continue;
            }
        ...
        if let Some(proof_id) = left.strip_prefix("proof:") {
            if !is_symbolic_name(proof_id) || !seen_proofs.insert(proof_id.to_string()) {
                errors.push(ValidationError::reject(
                    ...
                    format!("line:{line_number:03}"),
                    format!("duplicate or invalid bootstrap proof {proof_id}"),
                ));
                continue;
            }
        ...
        match left {
            ...
            _ => errors.push(ValidationError::reject(
                ErrorCode::InvalidEntrySyntax,
                format!("line:{line_number:03}"),
                format!("unknown bootstrap deployment key {left}"),
            )),
        }
    }
    if phase.is_none() {
...
pub fn validate_bootstrap_deployment_surface(input: &str) -> (Verdict, Receipt) {
    let canonical = canonical_surface_text(input).unwrap_or_else(|_| String::new());
    let mut errors = Vec::new();
    scan_forbidden_text(&canonical, &mut errors);
    match parse_bootstrap_deployment_surface(input) {
        Ok(surface) => validate_bootstrap_deployment_model(&surface, &mut errors),
...
pub fn validate_bootstrap_deployment_model(
    surface: &BootstrapDeploymentSurface,
    ...
    if surface.task != "P02-020" {
        errors.push(ValidationError::reject(
            ErrorCode::InvalidTask,
            "task",
            format!("expected P02-020 got {}", surface.task),
        ));
    }
    validate_status("surface", "P02-020", 0, &surface.status, errors);

    for rule in REQUIRED_BOOTSTRAP_DEPLOYMENT_RULES {
        match surface.rules.get(*rule) {
            ...
            None => errors.push(ValidationError::reject(
                ErrorCode::MissingDeploymentRule,
                format!("rule:{rule}"),
                "missing bootstrap deployment rule",
            )),
        }
    }
    ...
    require_ids(
        "target",
        REQUIRED_BOOTSTRAP_DEPLOYMENT_TARGETS,
        surface
            .targets
            .iter()
    ...
    require_ids(
        "hook",
        REQUIRED_BOOTSTRAP_COMPLIANCE_HOOKS,
    ...
    require_ids(
        "proof",
        REQUIRED_BOOTSTRAP_DEPLOYMENT_PROOFS,
    ...
    if !bootstrap_deployment_artifacts_bind_paths()
        || !bootstrap_deployment_hooks_bind_targets()
        || !bootstrap_deployment_evidence_bind_registry()
        || !bootstrap_deployment_proofs_bind_registry()
        || !bootstrap_deployment_receipts_cover_p02_001_through_p02_020()
        || !bootstrap_deployment_no_forbidden_descriptor_claims()
        || bootstrap_deployment_registry_hash().is_empty()
        || bootstrap_deployment_carrier_signature().is_empty()
    {
        errors.push(ValidationError::reject(
            ErrorCode::DeploymentDriftAccepted,
            "lyralang",
            "bootstrap deployment registry failed binding checks",
        ));
    }

    let report = deterministic_bootstrap_deployment_suite_report(
        &surface
            .targets
            .iter()
            .map(|item| {
        ...
        || !report.suite_hash.starts_with("fnv1a128:")
    {
        errors.push(ValidationError::reject(
            ...
            "bootstrap deployment report drift",
        ));
    }
}
...
fn parse_target(
    ...
    id: &str,
    value: &str,
) -> Result<BootstrapDeploymentTarget, ValidationError> {
...
fn parse_hook(
    ...
    id: &str,
    value: &str,
) -> Result<BootstrapComplianceHook, ValidationError> {
    let fields = parse_field_map(value).ok_or_else(|| {
        ValidationError::reject(
...
fn parse_evidence(
    ...
    id: &str,
    value: &str,
) -> Result<BootstrapReleaseEvidence, ValidationError> {
    let fields = parse_field_map(value).ok_or_else(|| {
        ValidationError::reject(
...
fn parse_proof(
    ...
    id: &str,
    value: &str,
) -> Result<BootstrapDeploymentProof, ValidationError> {
    let fields = parse_field_map(value).ok_or_else(|| {
        ValidationError::reject(
...
fn validate_targets(surface: &BootstrapDeploymentSurface, errors: &mut Vec<ValidationError>) {
    for target in &surface.targets {
        ...
        if let Some(descriptor) = bootstrap_deployment_target_descriptor(&target.id) {
            let digest = bootstrap_deployment_target_digest(&target.id).unwrap_or_default();
...
fn validate_hooks(surface: &BootstrapDeploymentSurface, errors: &mut Vec<ValidationError>) {
    for hook in &surface.hooks {
        ...
        if surface.target_by_id(&hook.target).is_none() && hook.target != "P02" {
            errors.push(ValidationError::reject(
                ErrorCode::InvalidComplianceHook,
                hook.canonical_identity(),
        ...
        if let Some(descriptor) = bootstrap_deployment_hook_descriptor(&hook.id) {
            let digest = bootstrap_deployment_hook_digest(&hook.id).unwrap_or_default();
...
fn validate_evidence(surface: &BootstrapDeploymentSurface, errors: &mut Vec<ValidationError>) {
    for evidence in &surface.evidence {
        ...
        if let Some(descriptor) = bootstrap_deployment_evidence_descriptor(&evidence.id) {
            let digest = bootstrap_deployment_evidence_digest(&evidence.id).unwrap_or_default();
...
fn validate_proofs(surface: &BootstrapDeploymentSurface, errors: &mut Vec<ValidationError>) {
    for proof in &surface.proofs {
        ...
        if let Some(descriptor) = bootstrap_deployment_proof_descriptor(&proof.id) {
            let digest = bootstrap_deployment_proof_digest(&proof.id).unwrap_or_default();
...
fn validate_command(
    ...
    if !REQUIRED_COMMANDS.contains(&command) {
        errors.push(ValidationError::reject(
            code,
            format!("line:{line_number:03}"),
            format!("unknown bootstrap deployment command {command}"),
        ));
    }
}
    ...
    for id in required {
        if !actual.contains(id) {
            errors.push(ValidationError::reject(
                code,
                format!("{kind}:{id}"),
                format!("missing bootstrap deployment {kind} {id}"),
            ));
        }
    }
}
    ...
    for (needle, code) in FORBIDDEN_DEPLOYMENT_TEXT {
        if lowered.contains(needle) {
            errors.push(ValidationError::reject(
                *code,
                "surface_text",
                format!("forbidden bootstrap deployment token {needle}"),
            ));
        }
    }
}
